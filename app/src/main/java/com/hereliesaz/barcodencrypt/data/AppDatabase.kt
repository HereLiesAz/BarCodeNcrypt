...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /**
         * Adds the indexed `rawValueHash` column so the scanner can resolve a scanned
         * barcode with a single indexed lookup instead of decrypting every row. The column
         * is nullable: pre-v13 rows are backfilled lazily on first scan (see
         * [BarcodeRepository.findBarcodesByRawValue]).
         */
        private val MIGRATION_12_13 = object : Migration(12, 13) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `barcodes` ADD COLUMN `rawValueHash` TEXT")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_barcodes_rawValueHash` ON `barcodes` (`rawValueHash`)")
            }
        }
            ...
            return INSTANCE ?: synchronized(this) {
                INSTANCE ?: run {
                    ...
                    val instance = Room.databaseBuilder(
                        ...
                        // Last-resort safety net for installs predating the earliest
                        // registered migration (schema v1/v2 have no migration path): wipe
                        // and rebuild rather than crash-loop on open.
