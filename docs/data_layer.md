...
*   **v3 (Planned):** To provide **post-compromise security**, the app will implement a **Double Ratchet** algorithm, inspired by the Signal protocol. This will combine a Diffie-Hellman (DH) ratchet for asynchronous key agreement with a symmetric-key ratchet for per-message keys. This will ensure that even if a key is compromised, the session can "heal" and become secure again after a few message exchanges. The implementation will use Google's Tink library for the underlying `X25519` elliptic curve cryptography.
...
*   **Passphrase Security:** The database is encrypted at rest using SQLCipher.
